...
if ($action == "list") {
    ...
    echo "<table class='table'><tr><th>#</th><th>Domain Name</th><th>Path</th><th>TomcatVersion</th><th>Status</th><th>Create Date</th><th>Ports</th><th>Action</th></tr>";
    ...
    while ($row = mysql_fetch_array($instanceResult)) {
        ...
        echo "<tr><td>$count</td><td>" . $row['domain_name'] . "</td>" ."<td>".$path."</td>". "<td>" . $row['tomcat_version'] . "</td><td>$status</td><td>" . $row['create_date'] . "</td><td>ShutDown Port:" . $row['shutdown_port'] . "<br/>HTTP Port:" . $row['http_port'] . "<br/>AJP Port:" . $row['ajp_port'] . "</td><td>";
        if ($row['status'] == "stop")
        ...
        if($row['delete_flag'] == 0) echo " <a class='btn btn-danger' href=#  onclick='deleteTomcatInstance(" . $row['id'] . ")' >Delete</a></td></tr>";
        $count++;
    }
